# 路线图

## P0 · MVP

| Phase | 内容 | 状态 |
|-------|------|------|
| 0 | 文档 bootstrap + git main 推远程 | ✅ 2026-08-03 |
| 0b | `android/` 空壳可 `assembleDebug` | ✅ 2026-08-03 |
| 1 | 几何/软边 · Repository · Cluster 上屏 · 首页三键+demo | 🔄 feature/p0-phase1-strip-display |
| 2 | 导入 · Cropper · 确认页 · 恢复示范 | ⬜ |
| 3 | FGS · 播歌让出 · pending · 开机恢复 | ⬜ |
| 4 | 定制/规格/关于 · 升级 CTA · Lyra queries | ⬜ |

## P0.5

| 项 | 状态 |
|----|------|
| 日夜双图完善 · 轮播 | ⬜ |
| handoff 导出包 · 检测 Lyra 自动让路 | ⬜ |
| 电池优化引导 | ⬜ |

## 1.1+

| 项 | 状态 |
|----|------|
| ContentProvider 迁移 · Lyra「从星澜导入」 | ⬜（Lyra 仓配合） |
| 主题包兑换码 · 多图库 | ⬜ |
| ring-wallpaper-core 共享 | ⬜ |

分支命名对齐：`feature/p0-phase1-strip-display` 等。见 [GIT_WORKFLOW.md](./GIT_WORKFLOW.md) · [TECH-NOTES-1.0.md](./TECH-NOTES-1.0.md)。
